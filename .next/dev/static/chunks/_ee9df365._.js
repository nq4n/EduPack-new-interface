...
async function getUserOwnedPackages(userId) {
    // Thanks to RLS, user only sees packages they own or are owner of.
    const { data, error } = await supabase.from("packages").select("package_id, title, description, created_at").order("created_at", {
        ascending: false
    });
    return {
        data: data,
...
async function getUserSubscription(userId) {
    const { data, error } = await supabase.from("billing").select("*").eq("billing_type", "subscription").order("created_at", {
        ascending: false
    }).limit(1).maybeSingle();
    return {
        data: data,
...
async function getUserCredits(userId) {
    const { data, error } = await supabase.from("billing").select("credit_value").eq("billing_type", "credit");
    const balance = data?.reduce((sum, row)=>sum + (row.credit_value ?? 0), 0) ?? 0;
    return {
        balance,
        error
    };
...
//# sourceMappingURL=_ee9df365._.js.map